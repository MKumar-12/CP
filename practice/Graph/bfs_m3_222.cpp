            ...
            while(!mod3_dist.empty()){
                ...
                for(auto neighbour : adjList[frontNode]){
                    if(indegree[neighbour] > 0){
                        indegree[neighbour]--;
                        if(indegree[neighbour] == 0)
                            mod3_dist.push(neighbour);
                        //mod3_dist[neighbour] = (mod3_dist[frontNode] + 1) % 3; 
                        for(int i = 0; i<=2 ; i++ ){
                            if(distance[i][frontNode] == true){
                                distance[neighbour][(i+1)%3]=true;
                            }

                        }
                        // if(mod3_dist[neighbour] == 0)
                        //     distance[0][neighbour] = true;
                        // else if(mod3_dist[neighbour] == 1)
                        //     distance[1][neighbour] = true;
                        // else
                        //     distance[2][neighbour] = true;
